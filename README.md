...
以當天為標準去檢查7天內快到期的合約，將快到期的合約顯示在主頁上，幫助業者掌握合約狀態。
...
載入PDF檔，並讀取PDF檔內容及Barcode，顯示內容再畫面上。

列印管理介面:

利用TXT文字檔匯入要作業的內容，讓使用者選擇要作業的品項，每列印一筆就把當前筆刪除，並搬移到作業紀錄裡，供使用者後續可進行查閱動作。

作業紀錄刪除介面:

讓使用者勾選要刪除的項目，排除不必要的紀錄內容。
